...
public class AnalyticsServiceImpl implements AnalyticsService {
	...
	private Concept getConcept(String s,Map<String,SemanticEntity> map) {
		...
		ret.setName(concept);
		String first=s.substring(s.indexOf('('), s.indexOf(','));
		String second=s.substring(s.indexOf(','), s.length()-1);
		SemanticEntity firstEntity=map.get(first),secondEntity=map.get(second);
	...
	private Collection<SemanticEntity> getEntities(Map<String,Set<String>> map) {
		...
		String[] keys={SemanticObservationService.WHO,SemanticObservationService.WHEN,SemanticObservationService.WHERE,SemanticObservationService.WHO};
		for(String s : keys) {
	...
	private Map<String,SemanticEntity> entities(Collection<SemanticEntity> entities) {
